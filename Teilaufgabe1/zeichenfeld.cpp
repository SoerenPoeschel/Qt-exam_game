...
void zeichenFeld::keyPressEvent(QKeyEvent *event)
{
    if(increment)
        switch (event->key()){
            ...
            break;
            case Qt::Key_Right:
            if (playerX+SQUARE_DIMENSION<WIDTH){
                playerX=playerX+15;}
            else playerX=playerX-1;
            break;
        default:
...
void zeichenFeld::save(QJsonObject &json) const
{
    json["count"]=count;
...
void zeichenFeld::load(const QJsonObject &json)
{
...
/*basic saving and loading with json: https://doc.qt.io/qt-5/qtcore-serialization-savegame-example.html
 * What has to be saved: count, counter, lifes, playerX, lastY1, lastY2, lastY3*/
